...
namespace WindowsFormsApp3
{
    public partial class DateWiseSalary : Form
    {
        ...
        private void btnshow_Click(object sender, EventArgs e)
        {
            ...
            {
                objadapt = new SqlDataAdapter("select * from Salary", objcon);
            }
            else
            {
                if (rbtndatewise.Checked)
                {
                    objadapt = new SqlDataAdapter("select * from Salary where Voucherdate>='" + dtpfromdate.Text + "' and Voucherdate<='" + dtptodate.Text + "'", objcon);
                }
            }
            objdt = new DataTable();
            objadapt.Fill(objdt);
